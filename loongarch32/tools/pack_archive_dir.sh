...
#使用示例：tools/pack_archive_dir.sh ${BASE_DIR}/workbase/overlaydir_strip/sysroot "sysroot" squashfs Yongbao 1.0 loongarch64 xz
	...
	pkg)
		case "${ARCHIVE_COMP_FORMAT}" in
			tar)
				...
				;;
			*)
				echo "指定的打包格式无法时别，将使用默认的tar进行打包。"
				ARCHIVE_TAR_FORMAT="--xz"
				ARCHIVE_TAR_SUFF="xz"
				;;
		esac
	...
	case "${ARCHIVE_MODE}" in
		...
		rawdisk)
			...
			pushd ${ARCHIVE_DIR} > /dev/null
				...
				ARCHIVE_DIR_USESPACE_COUNT=$(expr ${ARCHIVE_DIR_USESPACE_DU} + \( \( ${ARCHIVE_DIR_USESPACE_DU} / 100 \) + 52 \))
